...
function SeoulBasketPage() {
  ...
  const goToSeoulLoad = () => {
    navigate("/portal/seoul/load", { state: { SeoulTrackGolbalData } });
    window.scrollTo(0, 0);
  };
  ...
  const SeoulTrackPostApi = async () => {
    ...
    const data: SeoulTrackItemData = {
      ...
      savedId: uniqueList,
      useCanStartVisitorsLocationOptions: OptionTwo,
      useChangeLocationOptions: OptionThree,
      useHotelOptions: OptionOne,
    };
          ...
          <div className="SeoulBasketAlertBox">
            <div className="SeoulBasketAlertInner">
              <div className="SeoulBasketAlertBoxTwo">
                ...
                <div className="SeoulBasketAlertTwoText">
                  필수 방문 예정이란?
                </div>
              </div>
              <div className="SeoulBasketAlertBoxThree">
                이 트랙에 대해 관광을 진행하며 관광객과 반드시 방문할 곳을
                뜻해요. 만약 3개 항목이 있다면 1곳은 필수 방문, 2곳은 관광객과
                협의 또는 요청에 따라 결정돼요. 방문 예정 항목은 한 곳만 지정할
                수 있어요.
              </div>
            </div>
          </div>
